...
func makeJAs(hand: Hand, jas: char): Hand =
  let newcards = hand.cards.replace('J', jas)
  return makeHand(newcards, hand.bid)
...
iterator hand_rank_order(hand: Hand): bool =
  for f in [isfive, isfour, isfull, isthree, istwopair, ispair]:
    yield CARDS.anyIt(f(makeJAs(hand, it)))
  yield true
...
# for hand in sorted_hands: echo hand
